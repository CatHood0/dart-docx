...
extension StylesExtension on Style {
  Map<String, dynamic>? buildBlockAttributesMap({
    ...
    final StyleConfigurator? blockConfigurator =
        getConfiguratorOrNull(localXmlKeyParagraphBlockAttrsNode);
    if (blockConfigurator == null || blockConfigurator.isInvalid) return null;
    final Map<String, dynamic> blocks = <String, dynamic>{};
    final StyleConfigurator listConfigurator =
        blockConfigurator.getConfigurator(localXmlKeyListNode);
    final StyleConfigurator indentConfigurator =
        blockConfigurator.getConfigurator(localXmlKeyIndentNode);
    final StyleConfigurator tabIndentConfigurator =
        blockConfigurator.getConfigurator(localXmlKeyTabNode);
    final StyleConfigurator alignConfigurator =
        blockConfigurator.getConfigurator(localXmlKeyAlignmentNode);
    final StyleConfigurator spacingConfigurator =
        blockConfigurator.getConfigurator(localXmlKeySpacingNode);

    if (!listConfigurator.isInvalid) {
      final String? codeNum = listConfigurator
          .getConfigurator(localXmlKeyListTypeNode)
          .value as String?;
      final String? numberIndentLevel = listConfigurator
          .getConfigurator(localXmlKeyListIndentLevelNode)
          .value as String?;
    ...
    if (!alignConfigurator.isInvalid) {
    ...
    if (computeIndents) {
      if (!indentConfigurator.isInvalid) {
        final String? rawLeftIndent = indentConfigurator.attributes?['w:left'];
        final String? rawRightIndent =
            indentConfigurator.attributes?['w:right'];
        ...
        if (indent != null) {
          ...
          final int indentValue =
              (rawCurrentIndent / context.defaultTabStop).truncate();
      ...
      if (!tabIndentConfigurator.isInvalid) {
    ...
    if (!spacingConfigurator.isInvalid) {
      ...
      if (userParse != null) {
        ...
        if (rawLine != null) {
          final double effectiveSpacing = rawLine / kDefaultSpacing.value;
  ...
  Map<String, dynamic>? buildInlineAttributesMap({
    ...
    final StyleConfigurator? inlineConfigurator =
        getConfiguratorOrNull(localXmlKeyParagraphInlineAttrsNode);
    if (inlineConfigurator == null || inlineConfigurator.isInvalid) return null;
    final Map<String, dynamic> inlines = <String, dynamic>{};
    final StyleConfigurator? fontFamilyNode =
        inlineConfigurator.getConfiguratorOrNull(localXmlKeyFontsNode);
    ...
    final StyleConfigurator? sizeNode =
        inlineConfigurator.getConfiguratorOrNull(localXmlKeySizeFontNode);
    final String? color = inlineConfigurator
        .getConfiguratorOrNull(localXmlKeyCharacterColorNode)
        ?.value as String?;
    final String? backgroundColor = inlineConfigurator
        .getConfiguratorOrNull(xmlBackgroundCharacterColorNode)
        ?.value as String?;
    final StyleConfigurator? underlineNode =
        inlineConfigurator.getConfiguratorOrNull(xmlUnderlineNode);
    final StyleConfigurator? boldNode =
        inlineConfigurator.getConfiguratorOrNull(xmlBoldNode);
    final StyleConfigurator? strikeNode =
        inlineConfigurator.getConfiguratorOrNull(xmlStrikethroughNode);
    // script
    final StyleConfigurator? scriptNode =
        inlineConfigurator.getConfiguratorOrNull(xmlScriptNode);
    final String? scriptValue = scriptNode?.value as String?;
    final String? highlightColor = inlineConfigurator
        .getConfiguratorOrNull(xmlHighlightCharacterColorNode)
        ?.value as String?;
    ...
    if (family != null) {
      final bool acceptFamily =
          context.acceptFontValueWhen?.call(family) ?? true;
      if (acceptFamily) {
        inlines['font'] = family;
      }
    }
    if (color != null) {
      final bool shouldAcceptColor =
          context.checkColor?.call(color) ?? isValidColor(color);
      ...
      if (shouldAcceptColor) {
        if (context.colorBuilder == null) {
          ...
          if (newColorV != null && newColorV.trim().isNotEmpty) {
            inlines['color'] = newColorV.startsWith('#')
    ...
    if (backgroundColor != null) {
      final bool shouldAcceptColor =
          context.checkColor?.call(backgroundColor) ??
      ...
      if (shouldAcceptColor) {
        if (context.colorBuilder == null) {
          inlines['background'] = backgroundColor.startsWith('#')
              ? backgroundColor.toUpperCase()
              : '#$backgroundColor'.toUpperCase();
        } else if (context.colorBuilder != null) {
          final String? newColorV = context.colorBuilder?.call(backgroundColor);
          if (newColorV != null && newColorV.trim().isNotEmpty) {
            inlines['background'] = newColorV.startsWith('#')
                ? newColorV.toUpperCase()
                : '#$newColorV'.toUpperCase();
          }
        }
    ...
    if (sizeAttr != null) {
      ...
      if (acceptSize) {
        final int? possibleLevel =
            context.shouldParserSizeToHeading?.call(sizeAttr);
